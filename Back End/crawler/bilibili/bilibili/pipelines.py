...
import bilibili.table  as table
	...
	def process_item(self, item, spider):
		...
		with table.get_session() as s:
			...
			if s.query(table.Movie).filter(f'{table.Movie.__tablename__}.url' == item['url']).first() == None:
				...
				#movie = table.Movie(url = item['url'], download = 0, analyzed = 0, title = item['title'], crawl_time = item['crawl_time'], watchtime = item['watchtime'], upload_time = item['upload_time'], promulgator = item['promulgator'], duration = item['duration'])
