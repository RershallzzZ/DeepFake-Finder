...
def get_keyframe(video_path, image_path):
    split = '/' if '/' in video_path else '\\'
    video_fn = video_path.split(split)[-1].split('.')[0]
    print("key",video_fn)
    ...
    print("frame save directory: " + image_path)
    ...
    if USE_LOCAL_MAXIMA:
        ...
        print("长度",len(frame_indexes))
        for i in frame_indexes:
            keyframe_id_set.add(frames[i - 1].id)

        plt.figure(figsize=(40, 20))
        plt.ylabel('Difference', fontdict={'family': 'Times New Roman', 'size': 50})
        plt.xlabel('Frame', fontdict={'family': 'Times New Roman', 'size': 50})
        plt.yticks(fontproperties='Times New Roman', size=40)
        plt.xticks(fontproperties='Times New Roman', size=40)
        plt.locator_params(numticks=100)
        plt.stem(sm_diff_array)
        plt.savefig(os.path.join(image_path, video_fn+'.png'))
